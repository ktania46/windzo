<template>
      ...
      <div class="wrapper-button w-full box-border mt-4">
        ...
        >
          ...
          <div class="mt-5 w-full">
            <input
              @change="makeStudentIdFieldDirty"
              type="number"
              style="color:white"
              v-model="text"
              size="10"
              ...
              <span v-if="(text!=='' && text<=0)" style="color:darkred">Must be Positive</span>
            <input
            @change="makeQuestionIdFieldDirty"
              type="number"
            ...
            />
            <span v-if="(questionId!=='' && questionId<=0)" style="color:darkred">Must be Positive</span>
            <br /><br />
            <div class="mr-5 flex align: center" style="align: center">
  <div style="border: 0px solid red; align: center">
    <button v-if="(text==='' && questionId==='') || (text!=='' && text<=0) || (questionId!=='' && questionId<=0)"
      style="cursor:default"
      @click="chooseStd"
      class="py-2.5 px-8 mr-2 mb-2 ml-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600"
    >
      Search
    </button> 
    ...
    <br>
    <span v-if="isQuestionIdFieldDirty && isStudentIdFieldDirty && text==='' && questionId===''" style="color:darkred">At east one of the fields must be filled</span>
  </div>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <td class="px-6 py-4">
              ...
              class="p-3 w-full bg-black dark:bg-gray-900 rounded-md outline-none focus:bg-gray-100 dark:focus:bg-gray-700"
                ...
                @change="pointsChanged(question, $event)"
  ...
  <div style="border: 0px solid red">
    <button 
     v-bind:class= "[!toBeChangedPoints.valid ? 'py-2.5 px-8 mr-2 mb-2 ml-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600' : 'py-2.5 px-8 mr-2 mb-2 ml-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-green-700']"
     :disabled="!toBeChangedPoints.valid"
     v-bind:style="[toBeChangedPoints.valid ? 'cursor:pointer': 'cursor:default']"
      style="margin-left: 45%; align: center"
      @click="this.submitNewPoints"    >
      Submit
    </button>
  </div>
</template>
      ...
      .then((response) => {
        ...
        for (let i = offst; i <= limit; i++) {
          if (
            ...
            questionIds.push(response.data["questions"][i - 1]["questionID"]);
            data.push({
...
export default {
  ...
  data() {
    return {
      ...
      isStudentIdFieldDirty:false,
      isQuestionIdFieldDirty:false,
      toBeChangedPoints: {
        mtr: "",
        QuestionIds: [],
        StdPoints: [],
  ...
  methods: {
    ...
    pointsChanged: function (question, e) {
      console.log(question["mtr"]);
      console.log(question["stdPoint"]);
      console.log(question["id"]);
      if (Number(question["stdPoint"]) <= Number(question["qPoint"])) {
        ...
        this.toBeChangedPoints.valid = false;
    ...
    updateValidity()
    {
      ...
      this.toBeChangedPoints.valid=true;
      this.student.forEach(element => {
        if(Number(element.stdPoint)>Number(element.qPoint))
        {
                this.toBeChangedPoints.valid=false;
        }
      });
    ...
    makeStudentIdFieldDirty()
    {
      this.isStudentIdFieldDirty= true; 
    },

    makeQuestionIdFieldDirty()
    {
      this.isQuestionIdFieldDirty= true;
    },
